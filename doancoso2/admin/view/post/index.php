				...
				<li>
					...
					<span class="text">
						<h3>
                                   <?php
                                   $db = Db::getInstance();
                                   $sql = $db->query("SELECT COUNT(*) AS total FROM tbl_post ");
                                   $post = $sql->fetch();
				...
				<li>
					...
					<span class="text">
						<h3>
                                   <?php
                                   $db = Db::getInstance();
                                   $sql = $db->query("SELECT COUNT(DISTINCT post_author) AS total FROM `tbl_post`");
                                   $post = $sql->fetch();
                                   if(isset($post['total']))     echo $post['total'];
                                   else echo " ";
                                   ?>
						...
						<tbody >
                                   ...
                                   foreach($posts as $post){?>
                                        <tr>
                                             ...
                                             <td style="color: green"><?= $post->post_author ;?></td>
