             ...
             <table class="cart-table">
                 ...
                 <tbody>
                     <?php
                        ...
                        tbl_oder,tbl_product,tbl_client WHERE 
                        ktbl_oder.product_id=tbl_product.product_id AND 
                        tbl_oder.client_id=tbl_client.client_id AND tbl_oder.client_id = '$client_id'");
                        $i = 0;
                        ...
                        if ($count > 0) {
                            while ($row_order = mysqli_fetch_array($sql_order)) {
                                ...
                                $total = $giakhuyenmai * $row_order['oder_quantity'];
                                $i++;

                        ?>
                             <tr>
                                 <td><?php echo $i ?></td>
                                 <td><?php echo $row_order['oder_code'] ?></td>
                                 <td><?php echo $row_order['client_name'] ?></td>
                                 <td><?php echo number_format($total) . ' VNĐ' ?></td>
                                 <td><?php echo $row_order['oder_status'] ?></td>
                             </tr>
                         <?php }
                        } else { ?>
                         <tr>
